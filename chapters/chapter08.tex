...
\label{equation0004}
\end{equation}
\listofequations{Вероятностна функция на биномно разпределение разпределение}
...
ggplot(data.frame(x=values, y=density)) + aes(x=x) + geom_histogram() + labs(x="Binomial Distributed Random Values ", y="Density")
